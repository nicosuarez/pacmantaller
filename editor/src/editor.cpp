...
void Editor::correr()
{
	...
	refXml -> get_widget("barraModo",pBarraModo);
	//Conexiones
	...
	pBarraModo -> set_sensitive(false);//Inicialmente no se puede seleccionar nada
	//Corremos la ventana
...
void Editor::funcItemCerrar()
{
	...
	if ( T != NULL ) 
	{
		...
		pLabelInfo -> set_text("");
		pBarraModo -> set_sensitive(false);
	}
}
