...
int Grafo::agregarVertice(Vertice *V,int fila,int columna)
{
	...
	else
	{
		...
		if ( mapVertices[id] != 0 ) //Existe alguien para conectar
		{
			if (mapVertices[id] -> conexionOeste()) 
				return ERROR;
		}
	}
			...
			else
			{
				...
				if ( mapVertices[id] != 0 ) //Existe alguien para conectar
				{
					if ( !(mapVertices[id] -> conexionEste()) ) //Si no es posible conectarlo
						return ERROR;
					RegArista R(idV+fila*cantColumnas+cantColumnas,idV,id);
					listaAux.push_back(R);//Coloco la arista correspondiente
				}
			}
		}
		else
		{
			...
			if ( mapVertices[id] != 0 ) //Existe alguien para conectar
			{
				if ( !(mapVertices[id] -> conexionEste()) ) //Si no es posible conectarlo
					return ERROR;
				//columna++;
				RegArista R(id+fila*cantColumnas+cantColumnas-1,idV,id);
				listaAux.push_back(R);//Coloco la arista correspondiente
			}
		}
	}
	else
	{
		...
		if ( mapVertices[id] != 0 ) //Existe alguien para conectar
		{
			if (mapVertices[id] -> conexionEste()) 
				return ERROR;
		}
	}
	...
	if (V -> conexionNorte() ) //Si es posible la conexion norte
	{
			...
			else
			{
				...
				if ( mapVertices[id] != 0 ) //Existe alguien para conectar
				{
					if ( !(mapVertices[id] -> conexionSur()) ) //Si no es posible conectarlo
						return ERROR;
					RegArista R((0*cantColumnas)+columna+0*cantColumnas,idV,id);
					listaAux.push_back(R);//Coloco la arista correspondiente
				}
			}
		}
		else
		{
			...
			if ( mapVertices[id] != 0 ) //Existe alguien para conectar
			{
				if ( !(mapVertices[id] -> conexionSur()) ) //Si no es posible conectarlo
					return ERROR;
				RegArista R(id+(fila-1)*cantColumnas+cantColumnas*2,idV,id);
				listaAux.push_back(R);//Coloco la arista correspondiente
			}
		}
	}
	...
	else
	{
		...
		if ( mapVertices[id] != 0 ) //Existe alguien para conectar
		{
			if (mapVertices[id] -> conexionNorte()) 
				return ERROR;
		}
	}
